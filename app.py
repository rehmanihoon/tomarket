                    ...
                    async with session.post(url=url, headers=headers, data=data, ssl=False) as response:
                        ...
                        if 'status' in play_game:
                            if play_game['status'] == 0:
                                self.print_timestamp(
                                    f"{Fore.BLUE + Style.BRIGHT}[ Game Started ][ Follow Telegram Channel Here @amaanhacker ]{Style.RESET_ALL}"
                                    f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
...
if __name__ == '__main__':
    try:
        ...
        banner = """
_    __  __    _        _    _   _   _   _    _    ____ _  _______ ____
   / \  |  \/  |  / \      / \  | \ | | | | | |  / \  / ___| |/ / ____|  _ \
  / _ \ | |\/| | / _ \    / _ \ |  \| | | |_| | / _ \| |   | ' /|  _| | |_) |
 / ___ \| |  | |/ ___ \  / ___ \| |\  | |  _  |/ ___ \ |___| . \| |___|  _ <
/_/   \_\_|  |_/_/   \_\/_/   \_\_| \_| |_| |_/_/   \_\____|_|\_\_____|_| \_\
        """
        print(Fore.CYAN + Style.BRIGHT + banner + Style.RESET_ALL)
        
        tomarket.print_timestamp(f"{Fore.MAGENTA + Style.BRIGHT}[ Join Our Channel: https://t.me/amaanhacker ]{Style.RESET_ALL}")

        account_files = [f for f in os.listdir() if f.startswith('accounts-') and f.endswith('.json')]
        account_files.sort(key=lambda x: int(re.findall(r'\d+', x)[0]))
        
        tomarket.print_timestamp(f"{Fore.MAGENTA + Style.BRIGHT}╔═══════════════════════════════════════════╗{Style.RESET_ALL}")
        tomarket.print_timestamp(f"{Fore.MAGENTA + Style.BRIGHT}║       Select an option [MADE BY AMAAN HACKER]       ║{Style.RESET_ALL}")
        tomarket.print_timestamp(f"{Fore.MAGENTA + Style.BRIGHT}╠═══════════════════════════════════════════╣{Style.RESET_ALL}")
